        ...
        case 'publisher':
          if (Array.isArray(metadata[keys[i]])) {
            this.metadata.publisher = String(
              (metadata[keys[i]][0] && metadata[keys[i]][0]['#']) || metadata[keys[i]][0] || ''
            ).trim();
          } else {
        ...
        case 'creator':
          if (Array.isArray(metadata[keys[i]])) {
            ...
            this.metadata.creatorFileAs = String(
              (metadata[keys[i]][0] && metadata[keys[i]][0]['@'] && metadata[keys[i]][0]['@']['opf:file-as']) ||
                this.metadata.creator
            ).trim();
          } else {
            this.metadata.creator = String(metadata[keys[i]]['#'] || metadata[keys[i]] || '').trim();
            this.metadata.creatorFileAs = String(
              (metadata[keys[i]]['@'] && metadata[keys[i]]['@']['opf:file-as']) || this.metadata.creator
            ).trim();
          }
          break;
        case 'date':
    ...
    for (var i = 0; i < branch.length; i++) {
      if (branch[i].navLabel) {
        ...
        if (branch[i].navLabel && typeof branch[i].navLabel.text == 'string') {
          title =
            (branch[i].navLabel && branch[i].navLabel.text) || branch[i].navLabel === branch[i].navLabel
  ...
  getChapter(id, callback) {
    this.getChapterRaw(
      ...
      function(err, str) {
        ...
        str = str.replace(/<script[^>]*?>(.*?)<\/script[^>]*?>/gi, function(o, s) {
          return '';
        });
